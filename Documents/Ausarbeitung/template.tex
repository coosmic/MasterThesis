...
Bei PointNet wird jeder der $n$ Punkt der Punktwolke mit einer $3x3$ Matrix transformiert deren Gewichte gelernt sind. 
Diese T-Net genannte Transformation wird angewandt um die Punktwolke in ein neues Koordinaten-System zu überführen.
Danach wird jeder Punkt nacheinander mittels eines Multi-Layer-Perceptron (MLP) mit zwei Layern der Größe $64$ in einen neuen Feature-Raum überführt. 
...
Die Punkte des Feature-Raums werden wieder mit einem T-Net der Größe $64x64$ transformiert. Danach wird wieder ein MLP (drei Layer $64$, $128$, $1024$) angewandt und mittels Max-Pooling eine globaler Feature-Vektor der Größe $1024$ ermittelt. 
Dieser Vektor wird mit dem lokalen Feature-Vektor für jeden Punkt kombiniert und die daraus entstehende Matrix der Größe $nx1088$ wird mittels zwei MLP-Layern in eine Matrix der Größe $nxm$ überführt, wobei $m$ die Anzahl möglichen Label sind.
Der erste dieser MLPs hat die drei Layer mit den Größen $512$, $256$ und $128$, der Zweite besteht aus zwei Layern der Größe $128$ und $m$.
...
TODO: Erläutern wie Skalierung geschätzt wird.
...
TODO: Erläutern wie Netze funktioniren.
...
und das letzte Netz schätzt auf Basis der kontextuellen Information mittels eines differnezierbaren SVD Layers.
...
In einem zweiten Schritt wurden die Verfahren auf konkreten Beispielen der Problemstellung getestet. Hier wurden exemplarisch je eine Avocado- und eine Bananan-Pflanze getestet. 
...
Die Neuronalen-Netze liefern keine guten Ergebnisse. Mit Aussnahme von RPM-Net. Diese liegen in etwa gleich auf mit den Ergebnissen die für ICP erreicht wurden und sind besser als die Ergebnisse von RICP.
...
In Tabelle \ref{tab:segmentation:normalization:compare} werden die Ergebnisse der Evaluation von PointNet++ auf nicht normaliserten Daten nach dem Training mit normaliserten Daten mit normalisierten Daten verglichen.
...
Wird die Evaluation eines ohne zufällig rotierte Punktwolken trainierte Netz mit zufällig rotierten Punktwolken ausgeführt sinkt der mIoU-Wert auf $0,6$ und Accuracy auf $0,875$
...
Ein weiterer Ansatz der in dieser Arbeit nicht verfolgt wurde ist ScaleLK. ScaleLK hat eine ähnliche Archtektur wie PoinNetLK allerdings wird neben Rotation und Translation auch eine Schätzung für die Skalierung geliefert.
