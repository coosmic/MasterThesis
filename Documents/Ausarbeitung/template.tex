...
SIFT ist der wohl bekannteste Ansatz und wird häufig als Goldstandard verwendet um Verfahren miteinander zu vergleichen. Es hat allerdings den Nachteil, dass es zu langsam für Echtzeitanwendungen ist.
...
ORB geht diesen Nachteil an, indem zuerst das Massezentrum des Bildes $C$ (Gleichung \ref{eq:sfm:orb:c}) und dessen Orientierung $\theta$ (Gleichung \ref{eq:sfm:orb:theta} )
...
Hierbei wird der Vektor der Länge $9$ in drei Teile zerlegt, welche die Zeilen von $\hat{F}$ bilden.%und so angeordnet, dass eine Matrix der Größe $3\times3$ entsteht.
...
$Q(a_j, P_i)$ ist die geschätzte Projektion des Punktes $P_i$ auf das Bild der Kamera $j$ und $d$ ist die Euklidische Distanz. $x_ij$ ist der detektierte Punkt auf dem Bild.
...
Als Detektor wird Hessian Affine Feature Detector (HA) genutzt. 
...
Die Idee dabei ist, dass Distanzen unter dem Schwellwert $\alpha$ die Kosten senken.
...
Da $r$ von Pflanze zu Pflanze variieren kann, besteht die Möglichkeit $r$ als Parameter dem Request hinzuzufügen.
...
Die Registrierungs-Ergebnisse mit iterativer Schätzung der Skalierung einer Messreihe ist in Abbildung \ref{fig:registration:banana} zu sehen. 
...
Sowohl beim Training als auch bei der Evaluation wird die Genauigkeit, der Loss und die durchschnittliche Intersection over Union (IoU). IoU wird über alle zu segmentierenden Klassen gemittelt.
Für jede Klasse $C$ wird dazu der Jaccard-Koeffizient $j_c(\hat{P_c},P_c)$ gebildet, wobei $\hat{P_c}$ die Menge an Punkten aus der Schätzung des Netzes, die als $C$ klassifiziert werden ist und $P_c$ die Menge der als $C$ klassifizierten Punkte aus dem Groundtruth ist.
...
Diesmal wird jedoch ein Großteil der Hintergrund-Punkte entfernt, indem nur das Zentrum der Punktwolke beim Training betrachtet wird. Die Ergebnisse sind in Abbildung \ref{fig:T12_3C_PS_WN_NR_OC} zu sehen.
Wie dort zu sehen ist, verbessern sich die Ergebnisse für das Training mit weniger Hintergrund deutlich. Ein Vergleich der Segmentierung mit und ohne Hintergrund ist in Abbildung \ref{fig:segmentation:compare} zu sehen.
...
Beim Training des Netzes mit drei Klassen sind starke Ausreißer in den Loss-Werten zu erkennen, die sich auch in der Accuracy und dem IoU - wenn auch nicht so stark - widerspiegeln.
...
Insbesondere beim Schätzen der Skalierung kommt es noch zu Fehlern, allerdings ähneln sich die Fehler in den einzelnen Messreihen und daher können die Ergebnisse trotzdem genutzt werden.
...
Hierbei wird die Archtiektur von RPM belassen, allerdings wird, nach dem die Rotation bekannt ist, die Skalierung, wie in den Gleichungen \ref{eq:icp_scale:1} - \ref{eq:icp_scale:2} beschrieben, berechenet.
...
Um das zu bewerkstelligen muss für jedes Merkmal ein weiteres Label hinzugefügt werden und die Datenbasis, die für das Training genutzt wird, um Punktwolken erweitert werden, die diese Merkmale auch beinhalten.
...
Lediglich die Registrierung der Punktwolke konnte nicht erfolgreich gelöst werden, was die davon abhängigen Funktionalitäten betrifft. Auch die Segmentierung des Hintergrundes kann noch weiter verbessert werden.
