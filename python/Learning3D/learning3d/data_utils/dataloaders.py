...
class RegistrationData(Dataset):
	def __init__(self, algorithm, data_class=ModelNet40Data(), partial_source=False, partial_template=False, noise=False, additional_params={}):
		...
		available_algorithms = ['PCRNet', 'PointNetLK', 'DCP', 'PRNet', 'iPCRNet', 'RPMNet', 'RPMNetScale', 'DeepGMR']
		...
		if self.algorithm == 'RPMNetScale':
			from .. ops.transform_functions import RPMNetScaleTransform
			self.transforms = RPMNetScaleTransform(0.8, True)
	...
	def __getitem__(self, index):
		...
		if 'use_masknet' in self.additional_params and self.additional_params['use_masknet']:
