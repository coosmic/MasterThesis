...
def compute_rigid_transform(a: torch.Tensor, b: torch.Tensor, weights: torch.Tensor):
	...
	a_centered_rot = torch.bmm(a_centered, rot_mat)
	#print(a_centered_rot.shape)
	
	sumA = torch.sum(torch.sum(a_centered_rot  *a_centered_rot, -1), -1)
	sumAB = torch.sum(torch.sum(b_centered  *a_centered_rot, -1), -1)
	scale = sumAB / sumA

	# Update Rotation Matrix
	scale_mat = torch.zeros([scale.shape[0], 3, 3], device=a_centered_rot.get_device())
	for i in range(scale.shape[0]):
		diagVec = torch.ones(3).to(scale.get_device()) *scale[i]
		scale_mat[i] = torch.diag(diagVec)
	...
	translation = -rot_mat @ (scale*centroid_a[:, :, None]) + centroid_b[:, :, None]
	#translation = -(rot_mat @ (centroid_a[:, :, None])) + centroid_b[:, :, None]

	transform = torch.cat((scale_mat, rot_mat, translation), dim=2)
	return transform


class RPMNetScale(nn.Module):
