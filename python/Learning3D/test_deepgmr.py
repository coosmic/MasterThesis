...
def test_one_epoch(device, model, test_loader, withAccuracy=False):
	model.eval()
	test_loss = 0.0
	pred  = 0.0
	count = 0
	...
	if withAccuracy:
		...
		print(cloud_registration_resulst.shape)
		print(template.shape)
		for i in range(template.shape[0]):
			nbrs = NearestNeighbors(n_neighbors=1, algorithm='ball_tree').fit(cloud_registration_resulst[i].cpu().detach().numpy())
			distances, indices = nbrs.kneighbors(template[i].cpu().detach().numpy())
			print(distances.shape)
			error = error + np.sum((distances/distances.shape[0]))

			#DEBUG
			#showClouds(cloud_registration_resulst[i].cpu().detach().numpy().T, template[i].cpu().detach().numpy().T)
