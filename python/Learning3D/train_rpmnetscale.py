...
from learning3d.models import RPMNetScale, PPFNet
...
def test_one_epoch(device, model, test_loader):
	...
	for i, data in enumerate(tqdm(test_loader)):
		...
		loss_val = FrobeniusNormLoss()(output['est_T'], igt) + RMSEFeaturesLoss()(output['est_R'])
...
def train(args, model, train_loader, test_loader, boardio, textio, checkpoint):
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if test_loss<best_test_loss:
			...
			torch.save(model.feat_extractor.state_dict(), 'checkpoints/%s/models/best_ptnet_model.t7' % (args.exp_name))

		torch.save(snap, 'checkpoints/%s/models/model_snap.t7' % (args.exp_name))
		torch.save(model.state_dict(), 'checkpoints/%s/models/model.t7' % (args.exp_name))
		torch.save(model.feat_extractor.state_dict(), 'checkpoints/%s/models/ptnet_model.t7' % (args.exp_name))
...
def options():
	...
	parser.add_argument('--batch_size_t', default=2, type=int,
						metavar='N', help='mini-batch size (default: 32)')
...
def main():
	...
	trainset = RegistrationData('RPMNetScale', ModelNet40Data(train=True, num_points=args.num_points, use_normals=True), partial_source=True, partial_template=True)
	testset = RegistrationData('RPMNetScale', ModelNet40Data(train=False, num_points=args.num_points, use_normals=True), partial_source=True, partial_template=True)
	train_loader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=args.workers)
	test_loader = DataLoader(testset, batch_size=args.batch_size_t, shuffle=False, drop_last=False, num_workers=args.workers)
	...
	# Create RPMNet Model.
	model = RPMNetScale(feature_model=PPFNet())
